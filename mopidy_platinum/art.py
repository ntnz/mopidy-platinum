...
# A single transparent GIF pixel -- served whenever art can't be resolved so a
# page full of thumbnails never turns into a wall of broken-image icons.
PLACEHOLDER = (
    "image/gif",
    base64.b64decode("R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBTAA7"),
)
...
def get_thumbnail(core, uri, size):
    """Return (content_type, bytes) for a uri's art, resized to fit size x size.

    Always succeeds -- falls back to a transparent placeholder if art can't
    be found, fetched, or decoded.
    """
    cache_key = (uri, size)
